...
<page>
  ...
  <p class="text-justify">Digite <tt>java -version</tt> em seu Prompt de Comandos ou Terminal para verificar se você está com a máquina virtual instalada e qual sua respectiva versão.</p>
  ...
  <p class="text-justify">Acesse http://leiningen.org/#install e leia as instruções para instalação do script. Em seguida, execute o comando <tt>lein -v</tt> para verificar a versão atual e baixar as dependências que serão usadas inicialmente.</p>

  <p class="text-justify">Durante a escrita desse livro, o Leiningen estava em sua versão <tt>2.9.0</tt>, mas não se preocupe se a sua versão for mais recente enquanto você estiver lendo o livro.</p>

  <p class="text-justify">Execute <tt>lein</tt> sem nenhum parâmetro para ver as opções de uso do Leiningen. Voltaremos a elas assim que explicarmos a sintaxe da linguagem.</p>
...
y = f(x)
  ...
  <p class="text-justify">Para calcular o dobro, teríamos <tt>y = f(x) = x * 2</tt>, ou <tt>y = x * 2</tt>.</p>

  <p class="text-justify">Para calcularmos o dobro de um número, onde internamente calcularíamos o valor de um parâmetro <tt>x</tt> multiplicado por <i>2</i>, teríamos:</p>
...
<index entry="index operando" />
  <p class="text-justify">Note que estamos usando uma notação diferente: primeiro vem o <i>operador</i> <tt>dobro</tt> e, em seguida, vem o <i>operando</i>, ou parâmetro, <i>21</i>. Chamamos isso de <i>notação prefixa</i>.</p>
...
<p class="text-justify">No Ruby, o cálculo acima utiliza internamente a notação prefixa onde <i>21</i> é um objeto, <tt>*</tt> é um método (ou uma mensagem, se preferir) e <i>2</i> é um parâmetro.</p>

<code>
<p class="text-justify">x = 21 * 2</p>

<p class="text-justify">y = 21.*(2)</p>
</code>
...
<p class="text-justify">No Terminal do seu sistema operacional, execute o comando <tt>lein repl</tt> para carregar o REPL do Clojure. Se for a primeira vez que você o abre, algumas dependências ainda serão baixadas para o seu computador.</p>

<p class="text-justify">Digite a instrução <tt>(+ 1 2)</tt> e pressione ENTER. A linha de baixo trará o resultado, <i>3</i>.</p>
...
<p class="text-justify">Quando quiser sair do REPL, digite <tt>exit</tt> e pressione ENTER.</p>
</box>

<p class="text-justify">Sabemos que <tt>dobro</tt> também é uma função. Para calcular <tt>dobro</tt>, usaríamos:</p>
...
<p class="text-justify">Para que possamos testar todos os exemplos desse capítulo, precisamos criar uma função chamada <tt>dobro</tt>, que recebe um parâmetro <tt>x</tt> e devolve seu valor multiplicado por dois.</p>

<p class="text-justify">Em Clojure, criamos uma função <tt>dobro</tt> da seguinte forma:</p>
...
<p class="text-justify">No primeiro caso, <tt>x</tt> e <tt>y</tt> continuariam sendo iguais, mas ambos teriam o valor <i>9</i>. No segundo caso, <tt>x</tt> seria igual a <i>7</i> e <tt>y</tt> seria igual a <i>9</i>.</p>
